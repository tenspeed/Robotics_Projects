...
\hline
\end{DoxyParams}
Servo.\-cpp is a simple file which allows a user to run a single hobby servo.$<$ Standard C library Input-\/output ports, special registers $<$ Interrupt handling functions $<$ Include header for serial port class\-C\-O\-N\-S\-T\-R\-U\-C\-T\-O\-R initializes P\-W\-M output for a R\-C servo motor. and sets pen height to a safe value. Servo output pins are A0 \& A1 
\begin{DoxyParams}{Parameters}
{\em p\-\_\-serial\-\_\-port} & Serial object so we can write to screen \\
\hline
\end{DoxyParams}
...
Set\-\_\-\-Angle sets the angle of the servo 
\begin{DoxyParams}{Parameters}
{\em angle} & sets the servo angle between 0 and 360 degrees \\
\hline
\end{DoxyParams}
