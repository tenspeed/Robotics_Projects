...
union TWIStatusReg TWIStatusReg = {0};            // TWI_statusReg is defined in TWI_Master.h
...
****************************************************************************/
unsigned char TWIStatus( void )
{
  while ( TWIBusy() );             				// Wait until TWI has completed the transmission.
  return ( TWI_state & 0b11111100);                // Return error state.
}
...
****************************************************************************/
void TWISend( unsigned char *msg, unsigned char msgSize)
{
  unsigned char temp;

  while ( TWIBusy() );             // Wait until TWI is ready for next transmission.
...
****************************************************************************/
unsigned char TWIGet( unsigned char *msg, unsigned char msgSize )
{
  unsigned char i;

  while ( TWIBusy() );             // Wait until TWI is ready for next transmission.

  if( TWIStatusReg.lastTransOK )               // Last transmission competed successfully.              
  {                                             
  ...
  return( TWIStatusReg.lastTransOK );                                   
}
...
SIGNAL(SIG_2WIRE_SERIAL)
{
  ...
  switch (TWSR)
  {
    ...
    case TWI_MTX_DATA_ACK:      // Data byte has been tramsmitted and ACK received
      ...
      {
        TWIStatusReg.lastTransOK = TRUE;                 // Set status bits to completed successfully. 
        // TWI_state = TWSR;
		TWCR = (1<<TWEN)|                                 // TWI Interface enabled
